#!/bin/bash
#
# Backs up the the most recent snapshot of the local cassandra directory to S3
...
echo "Backing up the most recent cassandra snapshot to S3 ..." | tee $OUTPUTCAP

do_fail()
{
    echo "Failed" | tee -a $OUTPUTCAP
    shout "Failure while backing up cassandra data files to S3" "Captured output: $NL2$(cat $OUTPUTCAP)"
    rm -f $OUTPUTCAP
    exit 1
}

SNAPSHOT=$(ls /mnt/|grep snap|sort|tail -n 1)
[[ "$SNAPSHOT" = "" ]] && echo "Found no snapshots in /mnt/" | tee -a $OUTPUTCAP && do_fail

echo "Piping $SNAPSHOT contents through tar through lz4 through s3cmd put ..." | tee -a $OUTPUTCAP
tar --create --atime-preserve --force-local --preserve-permissions --one-file-system --recursion -f - /mnt/$SNAPSHOT | lz4c -9 | s3cmd --bucket-location=us-west-2 --reduced-redundancy put - s3://myproject-cassandra/${INSTANCE_NAME// /_}/$SNAPSHOT.tar.lz4 2>&1 | tee -a $OUTPUTCAP
[[ "$?" != "0" ]] && do_fail
